...
public class SolrDao {
   ...
   * @return The list containing the required number of random records (or fewer if the dataset doesn't have that many).
  ...
  private static SampleRecord convert(SolrDocument result) {
    ...
    final Instant lastIndexedTime = toInstant(result.getFieldValue(TIMESTAMP_UPDATE_FIELD));
  ...
  /**
   * Determines the last time that there was an update (i.e. a record was indexed). We do this by returning the maximum of all
   * update timestamps.
   *
  ...
  public Instant getLastUpdateTime() throws PersistenceException {
    ...
    return executeQuery(solrQuery).stream().findFirst()
                                  .map(document -> toInstant(document.getFieldValue(TIMESTAMP_UPDATE_FIELD)))
  ...
  private static Instant toInstant(Object value) {
    return value instanceof Date date ? date.toInstant() : Instant.EPOCH;
  }
}
