...
public final class ReportingEngine {
  ...
  /**
   * Get a summary of datasets for the given filters by finding records processed by the Clio Link Checking Service.
   *
  ...
  /**
   * Sanitizes a CSV field to mitigate CSV injection attacks. If the value starts with any of the characters =, +, -, @ it will be
   * prefixed with a single quote ('). Null values are preserved.
   */
   ...
   * @throws PersistenceException the persistence exception
   */
  public List<DatasetCheckSummary> findDatasetCheckSummary(FieldFilters clioFilters) throws PersistenceException {
    return new DatasetDao(reportingEngineConfiguration.sessionFactory()).findDatasetCheckSummary(clioFilters);
  }
}
